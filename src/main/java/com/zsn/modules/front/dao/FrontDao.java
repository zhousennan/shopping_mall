...
public interface FrontDao {
    ...
    @Insert("insert into order_info   (product_name,product_total_price,order_number,order_product_id,user_name,order_payment_status,trading_time,image_main,express_delivery_company,deleted_status,order_deliver_goods_status,order_status,order_id,address,consignee,user_phone) values " +
            "(#{productName},#{productTotalPrice},#{orderNumber},#{orderProductId},#{userName},#{orderPaymentStatus},#{tradingTime},#{imageMain},#{expressDeliveryCompany},#{deletedStatus},#{orderDeliverGoodsStatus},#{orderStatus},#{orderId},#{address},#{consignee},#{userPhone})")
